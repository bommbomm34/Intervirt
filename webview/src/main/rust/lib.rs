...
use wry::{ProxyEndpoint, WebViewBuilder};
...
pub fn create_webview(
    ...
    #[cfg(target_os = "linux")]
    {
        return run_on_gtk_thread(move || {
            create_webview_inner(parent_handle, width, height, url, None, proxy, nav_handler)
        });
    }
...
pub fn create_webview_with_user_agent(
    ...
    {
        return run_on_gtk_thread(move || {
            create_webview_inner(parent_handle, width, height, url, user_agent, proxy, nav_handler)
        });
    }
